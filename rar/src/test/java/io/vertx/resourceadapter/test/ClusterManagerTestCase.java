...
public class ClusterManagerTestCase {
  ...
  /**
   * We use ProgrammableClusterManagerFactory to load customer hazelcast
   * configuration file.
   */
  @Test
  public void testUsingProgrammableClusterManagerFactory() {
    // ServiceLoader<HazelcastClusterManager> cluterManagerFactoryLoader =
    // ServiceLoader.load(HazelcastClusterManager.class);
    // Assert.assertNotNull(cluterManagerFactoryLoader);
    // Assert.assertTrue(cluterManagerFactoryLoader.iterator().hasNext());
    // HazelcastClusterManager clusterManagerFactory =
    // cluterManagerFactoryLoader.iterator().next();
